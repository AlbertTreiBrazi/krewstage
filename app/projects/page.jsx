    ...
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: '32px 24px' }}>
      ...
      {loading ? <div className="spinner" /> : projects.length === 0 ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 14 }}>
          {projects.map(p => <ProjectCard key={p.id} project={p} onClick={() => router.push(`/projects/${p.slug || p.id}`)} />)}
        </div>
      )}
    </div>
  )
...
function ProjectCard({ project: p, onClick }) {
  ...
  return (
    <div className="card card-hover" onClick={onClick} style={{ padding: 0, overflow: 'hidden' }}>
      ...
      <div style={{ height: 100, position: 'relative', overflow: 'hidden', background: p.image_url ? 'transparent' : 'linear-gradient(135deg, var(--bg3) 0%, var(--card2) 100%)' }}>
        {p.image_url
          ? <img src={p.image_url} alt={p.title} style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center', display: 'block' }} />
          : <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32, opacity: 0.3 }}>
              {{'collab':'🎵','live_gig':'🎤','band_search':'🎸','session':'🎹'}[p.project_type] || '🎵'}
            </div>
        }
        {p.owner && (
          <div style={{ position: 'absolute', bottom: -14, left: 14, width: 30, height: 30, borderRadius: '50%', border: '2px solid var(--card)', overflow: 'hidden', background: 'var(--card)' }}>
            <Avatar profile={p.owner} size={30} />
          </div>
        )}
      </div>
